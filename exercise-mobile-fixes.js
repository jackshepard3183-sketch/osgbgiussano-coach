(function(){
  ...
  window.openExerciseDetail=async function(id){
    ...
    let sourceImage='';
    if(x.source_image_path&&client){
      const {data}=await client.storage.from('exercise-images').createSignedUrl(x.source_image_path,3600);
      if(data?.signedUrl)sourceImage=`<div class="card exercise-source-card" style="margin-top:12px"><b>Immagine originale</b><img src="${esc(data.signedUrl)}" alt="Fonte esercizio" class="exercise-source-image"></div>`;
    }
