  ...
  window.openPlayerStats=async function(id){
    ...
    try{
      ...
      modal(`<div class="mh"><h3>${esc([p.first_name,p.last_name].filter(Boolean).join(' '))}</h3><button class="close" onclick="closeM()"><i data-lucide="x"></i></button></div>
        ${verify}
        <div class="card"><div class="muted">ANAGRAFICA</div><h3>${esc([p.first_name,p.last_name].filter(Boolean).join(' '))}</h3><p class="muted">Anno ${p.birth_year||'—'} · ${p.active?'Attivo':'Non attivo'}</p><p class="muted">Ultima sincronizzazione: ${esc(syncText)}</p><div class="row" style="margin-top:10px"><button class="btn" onclick='openPlayerReport(${JSON.stringify(String(id))})'><i data-lucide="file-text"></i>Report bambino</button></div></div>
