(function(){
  ...
  async function loadEvents(){
    if(!client)return;
    const {data,error}=await client.from('events').select('id,event_date,start_time,meeting_time,title,event_type,team_color,opponent,home_away,venue,address,notes,score_for,score_against,post_match_notes').in('event_type',DB_TYPES).order('event_date',{ascending:true}).order('start_time',{ascending:true});
    if(error){console.error(error);return;}
    S.events=(data||[]).map(r=>({id:r.id,d:dd(r.event_date),date:r.event_date,time:hh(r.start_time),meet:hh(r.meeting_time),t:r.title,team:r.team_color||'ALL',place:r.venue||'',addr:r.address||'',type:TO_LABEL[r.event_type]||r.event_type,opponent:r.opponent||'',homeAway:r.home_away||'',notes:r.notes||'',scoreFor:r.score_for,scoreAgainst:r.score_against,postMatchNotes:r.post_match_notes||''}));
