(function(){
  ...
  async function loadStats(){
    ...
    for(const r of (data||[])){
      const s=next[r.player_id]||(next[r.player_id]=blank());
      s.total++;if(r.team_color==='BLU')s.BLU++;if(r.team_color==='GIALLA')s.GIALLA++;
    }
  ...
  window.team=function(){
    const html=typeof oldTeam==='function'?oldTeam():'';
    return `${html}<div class="card" style="margin-top:12px"><b>Partecipazione alle gare</b><p class="muted">Controlla l’equilibrio delle convocazioni tra tutti i bambini.</p><button class="btn alt" onclick="openCallupBalance()"><i data-lucide="scale"></i>Equità convocazioni</button></div>`;
  };

  const oldPlayer=window.openPlayerStats;
  window.openPlayerStats=function(id){
    if(typeof oldPlayer==='function')oldPlayer(id);
    const s=stats[id]||blank();
    const sheet=document.querySelector('#m .sheet');
    if(sheet)sheet.insertAdjacentHTML('beforeend',`<div class="card" style="margin-top:12px"><div class="muted">PARTECIPAZIONE GARE</div><div class="kpi">${s.total}</div><p class="muted">BLU ${s.BLU} · GIALLA ${s.GIALLA}</p></div>`);
    if(window.lucide)lucide.createIcons();
  };

  const oldAsg=window.asg;
  window.asg=async function(playerId,team){if(typeof oldAsg==='function')await oldAsg(playerId,team);await loadStats();};
