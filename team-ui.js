(function(){
  ...
  window.builder=function(){
    ...
    const row=p=>`<div class="assign"><div class="av">${p.seq||P.indexOf(p)+1}</div><div style="flex:1"><b>${esc(p.n)}</b><br><small class="muted">${esc(profile(p))}${p.sourceMissing?' · Da verificare':''}</small></div><div class="acts"><button class="mb" onclick='asg(${JSON.stringify(String(p.id))},"BLU")'>BLU</button><button class="my" onclick='asg(${JSON.stringify(String(p.id))},"GIALLA")'>GIALLA</button><button class="mn" onclick='asg(${JSON.stringify(String(p.id))},null)'>×</button></div></div>`;
