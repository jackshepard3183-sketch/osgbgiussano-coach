(function(){
  ...
  function empty(){return {present:0,absent:0,late:0,unavailable:0,total:0,pct:0};}

  async function loadStats(){
    if(!client)return;
    const {data,error}=await client.from('attendance').select('player_id,status').order('updated_at',{ascending:false});
  ...
  window.team=function(){
    ...
    const rows=P.length?P.map((p,i)=>{const s=stats[p.id]||empty();return `<div class="player" onclick='openPlayerStats(${JSON.stringify(String(p.id))})' style="cursor:pointer"><div class="av">${p.seq||i+1}</div><div class="meta"><b>${esc(p.n)}</b><br><small>Presenze ${s.pct}% · ${s.present} P · ${s.absent} A · ${s.late} R</small></div><span class="pill ${a[p.id]==='GIALLA'?'yellow':''}">${esc(a[p.id]||'Non assegnato')}</span></div>`;}).join(''):`<p class="muted">Rosa non disponibile.</p>`;
    return `${ttl('Squadra',`${P.length} bambini · BLU e GIALLA dinamiche per ogni gara`)}<div class="grid g3"><div class="card"><div class="muted">ROSA</div><div class="kpi">${P.length}</div></div><div class="card"><div class="muted">ASSEGNATI ORA</div><div class="kpi">${assigned}</div></div><div class="card"><div class="muted">NON ASSEGNATI</div><div class="kpi">${Math.max(0,P.length-assigned)}</div></div></div><div class="card" style="margin-top:12px"><p class="muted">Le squadre vengono composte per ogni singola gara.</p><button class="btn" onclick="go('builder')"><i data-lucide="users-round"></i>Componi BLU / GIALLA</button></div><div class="section">ROSA</div>${rows}`;
  };
  ...
  window.builder=function(){
    ...
    return `${ttl('BLU / GIALLA','Composizione dinamica della giornata')}${noMatch}<div class="grid g3"><div class="card"><b>BLU</b><div class="kpi">${g.BLU.length}</div></div><div class="card yellow"><b>GIALLA</b><div class="kpi">${g.GIALLA.length}</div></div><div class="card"><b>Non assegnati</b><div class="kpi">${g.N.length}</div></div></div><div class="card" style="margin-top:12px"><b>Non assegnati: ${g.N.length}</b>${g.N.map(row).join('')}</div><div class="builder" style="margin-top:12px"><div class="box b"><h3>BLU · ${g.BLU.length}</h3>${g.BLU.map(row).join('')}</div><div class="box y"><h3>GIALLA · ${g.GIALLA.length}</h3>${g.GIALLA.map(row).join('')}</div></div>`;
  };
