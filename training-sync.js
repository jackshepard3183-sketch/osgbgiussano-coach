(function(){
  ...
  async function loadSessions(){
    if(!client)return;
    const {data,error}=await client.from('training_sessions').select('id,session_date,title,duration_minutes,objective,intensity,style,notes,structure,player_count,coach_count,created_at').order('session_date',{ascending:false}).order('created_at',{ascending:false});
  ...
  function buildStructure(total,objective,players,coaches){
    ...
    if(coaches>=2&&players>=8&&total-used>=35){
      ...
      blocks.push({order:blocks.length+1,title:'Circuito a stazioni',duration_minutes:stationDuration,source:'stations',rotation_minutes:rotation,groups,stations});
      used+=stationDuration;
    }else{
      ...
      for(const ex of usable){
        ...
        blocks.push({order:blocks.length+1,...exercisePayload(ex),duration_minutes:d,source:'library'});
        used+=d;libraryUsed+=d;
  ...
  function weekSummary(){
    const now=new Date();const start=new Date(now);start.setDate(now.getDate()-((now.getDay()+6)%7));start.setHours(0,0,0,0);const end=new Date(start);end.setDate(start.getDate()+7);
  ...
  function structureHtml(structure){
    return structure.map(x=>{
      if(x.source==='stations'){
        const stations=(x.stations||[]).map(s=>`<div class="card" style="margin-top:8px"><b>Stazione ${s.station} · ${s.group_size} bambini</b><p style="margin:6px 0 0">${esc(s.title)}</p>${s.space?`<small class="muted">${esc(s.space)}</small>`:''}</div>`).join('');
        return `<div class="event"><div class="date">${x.duration_minutes} min</div><div class="body"><strong>${esc(x.title)}</strong><small>${x.groups?.join('–')||''} bambini · cambio ogni ${x.rotation_minutes||'—'} min</small>${stations}</div></div>`;
      }
      return `<div class="event"><div class="date">${esc(x.duration_minutes||'')} min</div><div class="body"><strong>${esc(x.title||'Esercizio')}</strong><small>${x.source==='library'?'Da archivio esercizi':'Blocco standard'}${x.space?' · '+esc(x.space):''}</small></div></div>`;
    }).join('');
  }
  ...
  window.train=function(){
    ...
    return `${ttl('Allenamenti','Crea, salva e riutilizza le sedute')}<div class="grid g4"><div class="card blue"><div class="muted">ROSA ATTIVA</div><div class="kpi">${P.length}</div><p>bambini disponibili</p></div><div class="card"><div class="muted">QUESTA SETTIMANA</div><div class="kpi">${w.count}</div><p>sedute · ${w.mins} min</p></div><div class="card"><div class="muted">ULTIMO OBIETTIVO</div><div class="kpi" style="font-size:28px">${esc(w.objective)}</div></div><div class="card yellow"><b>Archivio esercizi</b><p class="muted">${exCount} esercizi disponibili</p><button class="btn alt" onclick="exercises()"><i data-lucide="library"></i>Apri esercizi</button></div></div><div class="section">SEDUTE SALVATE</div><div class="card">${list}</div><button class="btn" style="margin-top:12px" onclick="trainingWizard()"><i data-lucide="sparkles"></i>Crea seduta</button>`;
  };

  window.trainingWizard=function(existing){
    const s=existing||{};
    modal(`<div class="mh"><h3>${existing?'MODIFICA ALLENAMENTO':'CREA ALLENAMENTO'}</h3><button class="close" onclick="closeM()"><i data-lucide="x"></i></button></div>
      <div class="field"><label>Data</label><input id="trDate" type="date" value="${esc(s.session_date||today())}"></div>
      ...
      <p class="muted">Con almeno 2 istruttori e 8 bambini il generatore crea automaticamente una fase a stazioni, distribuendo i gruppi in modo equilibrato.</p>
      <button class="btn" onclick="genTrain(${existing?JSON.stringify(String(s.id)):'null'})"><i data-lucide="sparkles"></i>${existing?'Rigenera struttura':'Genera struttura'}</button>`);
  };
  ...
  window.openTraining=function(id){
    ...
    modal(`<div class="mh"><h3>${esc(s.title)}</h3><button class="close" onclick="closeM()"><i data-lucide="x"></i></button></div><p>${fmt(s.session_date)} · ${s.duration_minutes||60} min</p><p class="muted">${s.player_count||P.length} bambini · ${s.coach_count||3} istruttori · ${esc(s.objective||'')}${s.intensity?' · Intensità '+esc(s.intensity):''}</p>${structureHtml(structure)}${s.notes?`<p class="muted">${esc(s.notes)}</p>`:''}<div class="row" style="margin-top:14px;gap:8px"><button class="btn alt" onclick='editTraining(${JSON.stringify(String(s.id))})'><i data-lucide="pencil"></i>Modifica</button><button class="btn alt" onclick='deleteTraining(${JSON.stringify(String(s.id))})'><i data-lucide="trash-2"></i>Elimina</button></div>`);
  };

  window.editTraining=function(id){const s=sessions.find(x=>String(x.id)===String(id));if(!s)return;closeM();trainingWizard(s);};
  window.deleteTraining=async function(id){const s=sessions.find(x=>String(x.id)===String(id));if(!s||!confirm(`Eliminare la seduta "${s.title}"?`))return;const {error}=await client.from('training_sessions').delete().eq('id',id);if(error){console.error(error);alert('Impossibile eliminare la seduta.');return;}closeM();await loadSessions();go('train');};
