(function(){
  ...
  window.train=function(){
    ...
    return `${ttl('Allenamenti','Crea, salva e riutilizza le sedute')}<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px"><button class="btn" onclick="trainingWizard()"><i data-lucide="sparkles"></i>Crea seduta</button><button class="btn alt" onclick="importTrainingPdf()"><i data-lucide="file-up"></i>Importa sedute da PDF</button></div><div class="grid g4"><div class="card blue"><div class="muted">ROSA ATTIVA</div><div class="kpi">${P.length}</div><p>bambini disponibili</p></div><div class="card"><div class="muted">QUESTA SETTIMANA</div><div class="kpi">${w.count}</div><p>sedute · ${w.mins} min</p></div><div class="card"><div class="muted">MODELLI</div><div class="kpi">${templates.length}</div><button class="btn alt" onclick="openTrainingTemplates()">Apri modelli</button></div><div class="card yellow"><b>Archivio esercizi</b><p class="muted">${exCount} esercizi disponibili</p><button class="btn alt" onclick="exercises()"><i data-lucide="library"></i>Apri esercizi</button></div></div><div class="section">SEDUTE SALVATE</div><div class="card">${list}</div>`;
  };
