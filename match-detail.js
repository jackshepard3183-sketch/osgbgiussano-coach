(function(){
  ...
  window.openMatch=function(id){
    ...
    S.match=e.id;
    const observations=e.postMatchNotes?`<div class="card" style="margin-top:12px"><div class="muted">OSSERVAZIONI DOPO LA GARA</div><p>${esc(e.postMatchNotes)}</p></div>`:'';
    modal(`<div class="mh"><h3>${esc(e.team||e.type||'GARA')}</h3><button class="close" onclick="closeM()"><i data-lucide="x"></i></button></div><h3>${esc(e.t||'Gara')}</h3><p>${fmt(e.date)}${e.time?' · '+esc(e.time):''}${e.meet?' · ritrovo '+esc(e.meet):''}</p><p class="muted">${esc(e.place||'')}${e.addr?'<br>'+esc(e.addr):''}</p>${e.notes?`<p class="muted">${esc(e.notes)}</p>`:''}${observations}<div class="row" style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px"><button class="btn" onclick="closeM();go('builder')"><i data-lucide="users-round"></i>Componi squadre</button><button class="btn yellow" onclick="closeM();messages()"><i data-lucide="message-circle"></i>WhatsApp</button><button class="btn alt" onclick='openMatchResult(${JSON.stringify(String(e.id))})'><i data-lucide="clipboard-pen-line"></i>Osservazioni</button><button class="btn alt" onclick='editMatch(${JSON.stringify(String(e.id))})'><i data-lucide="pencil"></i>Modifica</button><button class="btn alt" onclick='deleteMatch(${JSON.stringify(String(e.id))})'><i data-lucide="trash-2"></i>Elimina</button></div>`);
  };
})();