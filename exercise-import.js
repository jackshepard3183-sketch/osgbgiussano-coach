(function(){
  ...
  window.saveImportedExercise=async function(){
    ...
    const row={owner_user_id:user.id,title:draft.title,category:window.osgbExerciseImport.normalizeCategory(draft.category,draft.title,draft.objective,draft.description),duration_minutes:duration,objective:draft.objective||null,space:draft.space||null,equipment:window.osgbExerciseImport.normalizeEquipment(draft.equipment)||null,description:draft.description||null,variants:draft.variants||null,notes:draft.notes||null,source_type:importSource,source_url:sourceUrl||null,source_ocr_text:draft.saveText?(draft.transcript||null):null,diagram_svg:window.__diagramSvg||null,imported_at:new Date().toISOString()};
  ...
  window.saveWebExercise=async function(){if(!client||!user)return;const title=textVal('webTitle');if(!title){alert('Inserisci il titolo.');return;}const objective=textVal('webObjective'),description=textVal('webDescription');const row={owner_user_id:user.id,title,category:window.osgbExerciseImport.normalizeCategory(textVal('webCategory'),title,objective,description),duration_minutes:parseInt(textVal('webDuration')||'0',10)||null,objective:objective||null,space:textVal('webSpace')||null,equipment:window.osgbExerciseImport.normalizeEquipment(textVal('webEquipment'))||null,description:description||null,variants:textVal('webVariants')||null,source_type:'web',source_url:window.__webImportUrl||null,source_ocr_text:window.__webImportText||null,imported_at:new Date().toISOString()};const {error}=await client.from('exercises').insert(row);if(error){console.error(error);alert('Impossibile salvare l’esercizio.');return;}window.__webImportUrl=null;window.__webImportText=null;if(typeof window.osgbReloadExercises==='function')await window.osgbReloadExercises();closeM();exercises();};
