(function(){
  ...
  function reportHtml(data){
    ...
    return `<div class="mh"><h3>REPORT STAGIONE 2026/2027</h3><button class="close" onclick="closeM()"><i data-lucide="x"></i></button></div>
      ...
      <div class="grid g4"><div class="card"><b>Bambini</b><div class="kpi">${t.players}</div></div><div class="card"><b>Allenamenti</b><div class="kpi">${t.trainings}</div><p class="muted">${t.minutes} min programmati</p></div><div class="card"><b>Gare</b><div class="kpi">${t.matches}</div></div><div class="card"><b>BLU / GIALLA</b><div class="kpi">${t.blu}/${t.gialla}</div></div></div>
  ...
  window.printSeasonReport=function(){
    ...
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Report OSGB Coach 2020</title><style>body{font-family:Arial,sans-serif;color:#17223b;padding:24px;font-size:12px}h1{font-size:22px;margin:0 0 4px}p{margin:3px 0 14px}.sum{display:flex;gap:10px;flex-wrap:wrap;margin:16px 0}.sum div{border:1px solid #ccd4e2;border-radius:8px;padding:10px 14px}table{width:100%;border-collapse:collapse;font-size:10px}th,td{border:1px solid #d8dee9;padding:6px;text-align:left}th{background:#f2f5fa}@media print{body{padding:0}}</style></head><body><h1>OSGB Giussano · Scuola Calcio 2020</h1><p>Report stagione 2026/2027</p><div class="sum"><div><b>Bambini</b><br>${t.players}</div><div><b>Allenamenti</b><br>${t.trainings}</div><div><b>Minuti programmati</b><br>${t.minutes}</div><div><b>Gare</b><br>${t.matches}</div><div><b>BLU / GIALLA</b><br>${t.blu} / ${t.gialla}</div></div><table><thead><tr><th>Bambino</th><th>Esperienza</th><th>Sviluppo</th><th>Pres.</th><th>Ass.</th><th>Rit.</th><th>%</th><th>Conv.</th><th>BLU</th><th>GIALLA</th></tr></thead><tbody>${rows}</tbody></table><p style="margin-top:14px">Documento gestionale interno. Nessuna classifica o valutazione competitiva.</p><script>window.onload=()=>window.print()<\/script></body></html>`);w.document.close();
  };
